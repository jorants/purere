#from test.support import bigmemtest, _2G, captured_stdout
# From test.support we only need the following:
...
cpython_only = pytest.mark.skip("Only for cpython")
...
class ReTests(unittest.TestCase):
    ...
    # We do not have a c api so we can not easily lock the buffer by hand.
    # It should also not lead to segfaults, whicvh was what the issue is about.
    @cpython_only
    ...
    def test_basic_re_sub(self):
        ...
        self.assertEqual(
            re.sub(r"\d+", self.bump_num, "08.2 -2 23x99y"), "9.3 -3 24x100y"
        )
        self.assertEqual(
            re.sub(r"\d+", self.bump_num, "08.2 -2 23x99y", 3), "9.3 -3 23x99y"
        )
        self.assertEqual(
            re.sub(r"\d+", self.bump_num, "08.2 -2 23x99y", count=3), "9.3 -3 23x99y"
        )
    ...
    # We simply recompile the pattern if the flags differ.
    @cpython_only
    ...
    def test_sub_template_numeric_escape(self):
        ...
        self.checkTemplateError(
            ...
            r"\400",
            "x",
            r"octal escape value \400 outside of " r"range 0-0o377",
            0,
        )
        self.checkTemplateError(
            "x",
            r"\777",
            "x",
            r"octal escape value \777 outside of " r"range 0-0o377",
            0,
        )
        ...
        self.assertEqual(re.sub("((((((((((y))))))))))(.)", r"\118", "xyz"), "xz8")
        self.assertEqual(re.sub("((((((((((y))))))))))(.)", r"\11a", "xyz"), "xza")
    ...
    def test_symbolic_refs(self):
        self.checkTemplateError(
            "(?P<a>x)", r"\g<a", "xx", "missing >, unterminated name", 3
        )
        self.checkTemplateError("(?P<a>x)", r"\g<", "xx", "missing group name", 3)
        self.checkTemplateError("(?P<a>x)", r"\g", "xx", "missing <", 2)
        self.checkTemplateError(
            "(?P<a>x)", r"\g<a a>", "xx", "bad character in group name 'a a'", 3
        )
        self.checkTemplateError("(?P<a>x)", r"\g<>", "xx", "missing group name", 3)
        self.checkTemplateError(
            "(?P<a>x)", r"\g<1a1>", "xx", "bad character in group name '1a1'", 3
        )
        self.checkTemplateError(
            "(?P<a>x)", r"\g<2>", "xx", "invalid group reference 2", 3
        )
        self.checkTemplateError("(?P<a>x)", r"\2", "xx", "invalid group reference 2", 1)
        with self.assertRaisesRegex(IndexError, "unknown group name 'ab'"):
            re.sub("(?P<a>x)", r"\g<ab>", "xx")
        self.assertEqual(re.sub("(?P<a>x)|(?P<b>y)", r"\g<b>", "xx"), "")
        self.assertEqual(re.sub("(?P<a>x)|(?P<b>y)", r"\2", "xx"), "")
        self.checkTemplateError(
            "(?P<a>x)", r"\g<-1>", "xx", "bad character in group name '-1'", 3
        )
        # New valid/invalid identifiers in Python 3
    ...
    def test_re_split(self):
        ...
        for string in (
            ...
            self.assertTypedEqual(
                re.split(b"(:+)", string), [b"", b":", b"a", b":", b"b", b"::", b"c"]
            )
        for a, b, c in (
            "\xe0\xdf\xe7",
            "\u0430\u0431\u0432",
    ...
    def test_qualified_re_split(self):
        ...
        self.assertEqual(
            re.split("(:)", ":a:b::c", maxsplit=2), ["", ":", "a", ":", "b::c"]
        )
        self.assertEqual(
            re.split("(:+)", ":a:b::c", maxsplit=2), ["", ":", "a", ":", "b::c"]
        )
    ...
    def test_match_getitem(self):
        ...
        self.assertEqual(
            "a1={a1} b2={b2} c3={c3}".format_map(m), "a1=a b2=None c3=None"
        )
    ...
    def test_re_groupref_exists(self):
        ...
        self.assertEqual(
            re.match(r"^(\()?([^()]+)(?(1)\))$", "a").groups(), (None, "a")
        )
        self.assertIsNone(re.match(r"^(\()?([^()]+)(?(1)\))$", "a)"))
        self.assertIsNone(re.match(r"^(\()?([^()]+)(?(1)\))$", "(a"))
        self.assertEqual(re.match("^(?:(a)|c)((?(1)b|d))$", "ab").groups(), ("a", "b"))
        self.assertEqual(
            re.match(r"^(?:(a)|c)((?(1)b|d))$", "cd").groups(), (None, "d")
        )
        self.assertEqual(re.match(r"^(?:(a)|c)((?(1)|d))$", "cd").groups(), (None, "d"))
        self.assertEqual(re.match(r"^(?:(a)|c)((?(1)|d))$", "a").groups(), ("a", ""))
    ...
    def test_re_groupref_overflow(self):
        from purere.constants import MAXGROUPS
    ...
    def test_re_groupref(self):
        self.assertEqual(re.match(r"^(\|)?([^()]+)\1$", "|a|").groups(), ("|", "a"))
        self.assertEqual(re.match(r"^(\|)?([^()]+)\1?$", "a").groups(), (None, "a"))
        self.assertIsNone(re.match(r"^(\|)?([^()]+)\1$", "a|"))
        self.assertIsNone(re.match(r"^(\|)?([^()]+)\1$", "|a"))
        self.assertEqual(re.match(r"^(?:(a)|c)(\1)$", "aa").groups(), ("a", "a"))
        self.assertEqual(re.match(r"^(?:(a)|c)(\1)?$", "c").groups(), (None, None))

        self.checkPatternError(r"(abc\1)", "cannot refer to an open group", 4)

    def test_groupdict(self):
        self.assertEqual(
            re.match("(?P<first>first) (?P<second>second)", "first second").groupdict(),
    ...
    def test_expand(self):
        ...
        self.assertEqual(
            re.match("(?P<first>first)|(?P<second>second)", "first").expand(
                r"\2 \g<second>"
            ),
            " ",
        )
    ...
    def test_repeat_minmax(self):
        ...
        self.checkPatternError(r"x{2,1}", "min repeat greater than max repeat", 2)
    ...
    def test_getattr(self):
        ...
        self.assertEqual(
            re.compile("(?i)(?P<first>a)(?P<other>b)").groupindex,
            {"first": 1, "other": 2},
        )
    ...
    def test_special_escapes(self):
        ...
        self.assertEqual(
            re.search(r"\b(b.)\b", "abcd abc bcd bx", re.ASCII).group(1), "bx"
        )
        self.assertEqual(
        ...
        self.assertEqual(re.search(br"\d\D\w\W\s\S", b"1aa! a").group(0), b"1aa! a")
        self.assertEqual(
    ...
    def test_named_unicode_escapes(self):
        ...
        self.checkPatternError(r"\N{SNAKE", "missing }, unterminated name", 3)
        self.checkPatternError(r"[\N{SNAKE]", "missing }, unterminated name", 4)
        self.checkPatternError(r"[\N{SNAKE]}", "undefined character name 'SNAKE]'", 1)
        self.checkPatternError(r"\N{SPAM}", "undefined character name 'SPAM'", 0)
        self.checkPatternError(r"[\N{SPAM}]", "undefined character name 'SPAM'", 1)
    ...
    def test_copying(self):
        ...
        m = p.match("12.34")
        # Removed below as match objects should probably actually be copied
    ...
    def test_flags(self):
        ...
        # removed L flag as this is not supported on purpose
    ...
    def test_sre_character_literals(self):
        ...
        self.checkPatternError(
            r"\567", r"octal escape value \567 outside of " r"range 0-0o377", 0
        )
    ...
    def test_sre_byte_literals(self):
        ...
        self.checkPatternError(
            br"\567", r"octal escape value \567 outside of " r"range 0-0o377", 0
        )
    ...
    def test_sre_byte_class_literals(self):
        ...
        self.assertRaises(re.error, re.compile, br"[\U00012345]")
        self.checkPatternError(
            br"[\567]", r"octal escape value \567 outside of " r"range 0-0o377", 1
        )
    ...
    def test_bug_418626(self):
        ...
        self.assertEqual(
            re.match(".*?cd", 5000 * "ab" + "c" + 5000 * "ab" + "cde").end(0), 20003
        )
    ...
    # Changed to pass the stict unicode flag that was added. without it isnumeric() is used and this fails
    def test_bug_6561(self):
        ...
        for x in decimal_digits:
            self.assertEqual(re.match(r"^\d$", x, re.STRICTUNI).group(0), x)
        ...
        for x in not_decimal_digits:
            self.assertIsNone(re.match(r"^\d$", x, re.STRICTUNI))
    ...
    def test_inline_flags(self):
        ...
        # For now we disable this, as the warning raised had the wrong stacklevel
        ...
        with self.assertWarns(DeprecationWarning) as warns:
            self.assertTrue(
                re.fullmatch("(^)?(?(1)(?i)" + upper_char + ")", lower_char)
            )
        self.assertRegex(str(warns.warnings[0].message), "Flags not at the start")
    ...
    def test_scoped_flags(self):
        ...
        self.checkPatternError(
            r"(?i-i:a)", "bad inline flags: flag turned on and off", 5
        )
        self.checkPatternError(
            r"(?au:a)", "bad inline flags: flags 'a', 'u' and 'L' are incompatible", 4
        )
        self.checkPatternError(
            br"(?aL:a)", "bad inline flags: flags 'a', 'u' and 'L' are incompatible", 4
        )
    ...
    def test_zerowidth(self):
        ...
        self.assertEqual(re.findall(r"\b|\w+", "a::bc"), ["", "a", "", "", "bc", ""])

        self.assertEqual(
            [m.span() for m in re.finditer(r"\b|:+", "a::bc")],
            [(0, 0), (1, 1), (1, 3), (3, 3), (5, 5)],
        )
        self.assertEqual(
...
"""
    ...
    def test_keyword_parameters(self):
        ...
        self.assertEqual(
            pat.split(string="abracadabra", maxsplit=1), ["", "ab", "racadabra"]
        )
        self.assertEqual(
            pat.scanner(string="abracadabra", pos=3, endpos=10).search().span(), (7, 9)
        )
    ...
    def test_bug_34294(self):
        ...
        self.assertEqual(
            [m.groups() for m in re.finditer(p, s)], [("a", "b"), ("b", None)]
        )
        ...
        self.assertEqual(
            [m.groupdict() for m in re.finditer(p, s)],
            [{"tag": "test", "text": "<foo2/>"}, {"tag": "foo2", "text": None}],
        )
        s = "<test>Hello</test><foo/>"
        self.assertEqual(
            [m.groupdict() for m in re.finditer(p, s)],
            [{"tag": "test", "text": "Hello"}, {"tag": "foo", "text": None}],
        )
...
class PatternReprTests(unittest.TestCase):
    ...
    def test_unknown_flags(self):
        self.check_flags(
            "random pattern", 0x123000, "re.compile('random pattern', 0x123000)"
        )
        self.check_flags(
            "random pattern",
            0x123000 | re.I,
    ...
    def test_quotes(self):
        self.check(
            'random "double quoted" pattern',
            """re.compile('random "double quoted" pattern')""",
        )
        self.check(
            "random 'single quoted' pattern",
